...
class Decoder(object):
    ...
    def __call__(self, input_indices, memory, valid_indices, left_indices, right_indices, valid_masks, init_state=None):
        ...
        seq_length = tf.shape(input_indices[0])[1]
        ...
        def step(state_tm, subtree_t, vd_idx_t, lt_idx_t, rt_idx_t, mask_t):
            ...
            # attention over the subtree
            att_input = tf.tanh(tf.expand_dims(tf.matmul(hid_t, weight_hid_input), 1) + \
                                tf.nn.conv1d(memory, weight_mem_input, 1, 'SAME') + \
                                bias_input)
            score_input = tf.reduce_sum(att_input * weight_input, [2])
            prob_input = tf.softmax(score_input * subtree_t)
            inp_t = tf.reduce_sum(tf.expand_dims(prob_input, dim=2) * memory, axis=1)
